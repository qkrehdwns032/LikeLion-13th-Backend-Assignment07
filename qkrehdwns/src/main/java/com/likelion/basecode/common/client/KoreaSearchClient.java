...
public class KoreaSearchClient {

    private final RestTemplate restTemplate;

    @Value("${korea-api.base-url}")
    private String baseUrl;

    @Value("${korea-api.service-key}")
    private String serviceKey;

    // 외부 도서 API로부터 전체 도서 목록을 조회
    public List<KoreaResponseDto> fetchAllKoreas() {
        URI uri = UriComponentsBuilder.fromUriString(baseUrl)
            .queryParam("serviceKey", serviceKey)
            .queryParam("numOfRows", 100) // 한 페이지에 100권 조회 (임의 세팅)
            .queryParam("pageNo", 1)  //
            .queryParam("keyword", "") // 시작 페이지 (임의 세팅)
            .build()
            .toUri();
        ...
        Map<String, Object> body = Optional.ofNullable(response.getBody())
            .orElseThrow(() -> new BusinessException(ErrorCode.KOREA_API_RESPONSE_NULL, ErrorCode.KOREA_API_RESPONSE_NULL.getMessage()));
    ...
    private List<Map<String, Object>> extractItemList(Map<String, Object> responseMap) {
        // 'item' 리스트까지 접근
        Map<String, Object> response = castToMap(responseMap.get("response"), ErrorCode.KOREA_API_BODY_MALFORMED);
        Map<String, Object> body = castToMap(response.get("body"), ErrorCode.KOREA_API_BODY_MALFORMED);
        Map<String, Object> items = castToMap(body.get("items"), ErrorCode.KOREA_API_ITEMS_MALFORMED);
        ...
        // 그 외는 형식 오류로 예외 처리
        throw new BusinessException(ErrorCode.KOREA_API_ITEM_MALFORMED, ErrorCode.KOREA_API_ITEM_MALFORMED.getMessage());
    }

    // 개별 도서 항목 Map을 BookResponseDto로 변환
    private KoreaResponseDto toDto(Map<String, Object> item) {
        return new KoreaResponseDto(
            (String) item.getOrDefault("title", ""),
            (String) item.getOrDefault("URL", ""),
            (String) item.getOrDefault("DESCRIPTION", ""),
            (String) item.getOrDefault("LOCAL_ID", "")
        );
    }
